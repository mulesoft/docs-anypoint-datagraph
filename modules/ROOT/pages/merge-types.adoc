...
In this example, because the Customer API was added first, the resulting type in the unified schema is called `Customer`, containing all the fields from both Customer and UserAccount types, and the resulting query method is called `customersById`, returning the merged `Customer` type:
...
You can also merge two types when only one of the types has collaboration enabled. In this case, the merging causes your current type (the type you are configuring to merge) to act as a _reference_ to the target type (the type you are merging with). Note that for the merge to happen, the target type must have collaboration enabled, and both types must define one primary key or use xref:merging-types-that-use-composite-keys[composite keys].
...
The types you merge may have query methods that use _composite keys_, which means the type uses two primary keys.
...
And a `SalesOrder` API that also returns a `Product` type but uses composite keys, `id` and `sku`. 
