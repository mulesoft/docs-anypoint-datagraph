...
+
image::composite-keys-default-query-method.png["Query method configuration interface with composite key selection"]
. Click *Next*.
...
+
image::composite-keys.png["Primary key configuration interface with multiple key selection"]
