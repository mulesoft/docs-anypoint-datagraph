...
+
For information, see xref:permissions.adoc[DataGraph Permissions].
...
+
image::datagraph-qsg-configure-api.png["API configuration interface for setting version and endpoint URL"]
+
...
+
image::datagraph-qsg-api-authentication.png["API authentication configuration panel with No Auth option selected"]
+
...
+
image::datagraph-qsg-preview-schema.png["API schema preview interface showing type definitions and relationships"]
+
...
+
image::datagraph-qsg-preview-search.png["Schema preview search interface with price element highlighted"]
...
+
image::datagraph-qsg-default-query-method.png["Query method configuration panel with product ID-based default method"]
+
...
+
image::datagraph-qsg-primary-key.png["Primary key configuration interface with identifier field selected"]
+
...
+
image::datagraph-qsg-add-api-status.png["Progress indicator displaying API schema integration status"]
+
...
image::datagraph-qsg-rename-type.png["Type renaming interface with enum type selection"]
...
+
image::datagraph-qsg-visible-object-type.png["Object type visibility configuration panel for order item summary"]
...
+
image::datagraph-qsg-link-configuration.png["Link configuration interface showing foreign key field mapping"]
+
...
+
image::datagraph-qsg-link-added.png["Order item summary interface displaying linked product type indicators"]
...
+
image::datagraph-qsg-merge-selection.png["Type merge configuration interface with order type selection"]
...
+
image::datagraph-qsg-merge-preview-result.png["Preview interface showing merged type structure"]
+
...
+
image::datagraph-qsg-request-query-access.png["Application creation interface for requesting query operation access"]

. Click *Next*.
. In the *Create a new application* window, complete the fields, then click *Next*:
+
image::datagraph-qsg-create-query-application.png["Query application configuration interface for access requests"]
...
+
image::datagraph-qsg-explore-schema-docs.png["Query editor interface with schema explorer panel expanded"]
+
...
+
image::datagraph-qsg-query-autocompletion.png["Query editor interface showing inline code completion suggestions"]
...
image::datagraph-qsg-mutation-result.png["Mutation execution results showing newly created order details"]
...
+
image::datagraph-qsg-two-apis.png["Query interface displaying results from multiple API sources"]
...
+
image::datagraph-qsg-query-result.png["Query results panel showing formatted response data"]
. To run the query with query tracing, from the actions menu (*...*), select *Trace Query*, and then click *Run*.
+
image::datagraph-qsg-query-traces.png["Query execution trace interface with detailed performance metrics"]
+
...
+
image::datagraph-qsg-response-logs.png["Query response logging interface with detailed request information"]
+
