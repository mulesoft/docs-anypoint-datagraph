...
+
image::datagraph-qsg-configure-api.png[Configuring the API and asset version and URL for the Catalyst Product API]
+
...
+
image::datagraph-qsg-preview-schema.png[Anypoint DataGraph preview schema view]
+
...
+
image::datagraph-qsg-default-query-method.png[Set the default query method to products by product id]
+
...
+
image::datagraph-qsg-primary-key.png[Set the primary key to identifier string]
+
...
+
image::datagraph-qsg-add-api-status.png[Anypoint DataGraph status indicator indicates adding API to graph,75%,75%]
+
...
image::datagraph-qsg-rename-type.png[Enum type 0 in the catalyst order API]
...
+
image::datagraph-qsg-visible-object-type.png[Setting the desired state to Visible]
...
+
image::datagraph-qsg-link-configuration.png[Setting the foreign key field in the link configuration]
+
...
+
image::datagraph-qsg-link-added.png[Product type field shows the linked icon]
...
+
image::datagraph-qsg-merge-selection.png[Selecting the target type for a merge]
...
+
image::datagraph-qsg-request-query-access.png[Requesting access to run a query,70%,70%]

. Click *Next*.
. In the *Create a new application* window, complete the fields:
+
image::datagraph-qsg-create-query-application.png[Creating a query application,70%,70%]
...
+
image::datagraph-qsg-query-result.png[DataGraph displays query results]
