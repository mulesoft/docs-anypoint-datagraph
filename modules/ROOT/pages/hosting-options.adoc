...
+
`Domain |  datagraph-<org_id_excerpt>-<env_id_excerpt>`
...
+
Replace `<url>` with a valid URL that includes the DLB name and DataGraph app name (gathered in Steps 3 and 4 of the gathering details task) and the mapping rule *Input Path*. 
+
...
|0 |`/graphql` |`datagraph-<org_id_excerpt>-<env_id_excerpt>` |`/graphql` |HTTPS
|1 |`/datagraph/` |`datagraph-<org_id_excerpt>-<env_id_excerpt>` |`/graphql` |HTTPS
|2 |`/datagraph/` |`datagraph-<org_id_excerpt>-<env_id_excerpt>` |`/` |HTTPS
|3 |`/{app}/` |`{app}` | `/` |HTTPS
|===
+
...
* Index 3: `examplelb.lb.anypointdns.net/datagraph-<org_id_excerpt>-<env_id_excerpt>/graphql`
+
If, for exampe, the *Input Path* matches Index 2, run: 
+
....
