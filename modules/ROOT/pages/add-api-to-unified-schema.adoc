...
Note that some elements in your API specification, such as non-GET operations, are not added to the API schema. See xref:troubleshoot-schemas-queries.adoc[Errors When Generating the API Schema] for more information.
...
NOTE: You can also use the xref:datagraph-cli.adoc[Anypoint CLI] to add and edit API sources in DataGraph. However, if you add an API source using the CLI, you cannot edit it or update its version directly in the UI. You must make these changes using the CLI.

== Before You Begin

To add an API to the unified schema, confirm the following:

* Your API is a REST API created using either a RAML or OAS specification, or is a GraphQL API.
* You have Published the API in Exchange in the same organization as your unified schema. DataGraph supports only `stable` state API versions. 
* You have initiated an instance of the API with an accessible URL.
