...
[IMPORTANT]
--
* Supported authentication methods to access your API from Anypoint DataGraph are listed in the following procedure. Any other authentication methods, such as custom OAuth implementations or short-lived token methods, are not supported. See xref:security.adoc#supported-authentication-methods[Supported Authentication Methods] for more information.
--
...
. Still on the *Configure Security* page, optionally configure mutual authentication (mTLS) between the API and DataGraph 
.. Select *Configure Keytore*, and provide the following:
...
.. Optionally, select *Configure Truststore* and provide a link to the root CA.
...
If you need to configure mTLS for an API that you've added to the unified schema, you can do so on the *API details* page.
