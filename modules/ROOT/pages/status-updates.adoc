...
image::datagraph-status-update-in-process.png["Progress indicator displaying API schema integration status"]
...
image::datagraph-status-update-finished.png["Status indicator confirming schema update completion"]

When all updates are complete, the status changes to *Up to date*.

image::status_uptodate.png["Status indicator confirming schema synchronization"]
