...
+
image::link-object-type.png["Link configuration interface showing type relationship mappings"]

. Click *Save changes*.

If you scroll up to the fields pane, an icon shows the fields are now linked:

image::linked-type-icon.png["Interface element showing linked type relationship indicators"]
