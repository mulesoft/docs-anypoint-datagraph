...
Anypoint DataGraph supports the following GraphQL features:
...
|`@include` |Yes
|`@deprecated` |Yes
|Custom directives |No
|===

=== Apollo GraphQL Directive Support

Anypoint DataGraph supports the following Apollo GraphQL directives:
...
Only the first `@key` is used and any others are ignored.
|`@provides` |Yes
|`@external` |Yes
|`@requires` |Yes
|`@shareable` |Yes
...
The `from` argument of the required directive is not considered. Any value annotated with `@override` can be used in the response without any particular precedence.
|`@inaccesible` |Yes
|===
 

== About DataGraph Unified Schema and Query Complexity

DataGraph supports the following:
