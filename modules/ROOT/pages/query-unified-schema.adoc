...
+
image::datagraph-query-variable.png["Query editor interface showing variable insertion options"]
...
+
image::datagraph-mutation-autocomplete.png["Query editor interface showing mutation code completion suggestions"]

. Optionally, in the **Query Variables** pane, add any variables you want to use.
. Click *Run*. 
+
image::datagraph-mutation-result.png["Mutation execution results showing operation outcome"]
. To see results with query tracing, from the actions menu (*...*), select *Trace Query* and then click *Run*.
. To see response logs associated with the query, from the actions menu (*...*), select *View Response Logs*.
...
image::confirm-credentials.png["External client authentication configuration interface"]
