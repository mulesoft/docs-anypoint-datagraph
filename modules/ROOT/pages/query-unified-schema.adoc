...
+
image::datagraph-qsg-request-query-access.png[Requesting access to run a query,70%]
+
...
+
image::datagraph-qsg-query-autocompletion.png[Query editor autocompletion,50%]

. (Optional) In the *Query Variables* pane, add any query variables you want to use.
+
image::datagraph-query-variable.png[Inserting a query variable,60%]

. Click *Run*.
+
image::datagraph-qsg-query-result.png[DataGraph displays the query results]
...
+
image::datagraph-qsg-query-response-logs.png[Query response logs]
+
...
== Use Aliases in DataGraph Queries

You can use aliases to rename individual keys in the JSON response:

[source]
--
{
ordersById(id:"J4RRYV0001") [
  origination
  tax : taxAmount //<1>
  orderDate
  totalAmount
...
<1> By using GraphQL aliases, you can rename the `tax` key to `taxAmount` in only your query response.
